...
def reproducir_mp3(ruta_mp3):
    ...
    try:
        subprocess.run(["ffmpeg", "-y", "-i", ruta_mp3, wav], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
async def hablar_edge(texto):
    ruta = None
    try:
        communicate = edge_tts.Communicate(text=f"{texto}...", voice="es-MX-DaliaNeural")
        with tempfile.NamedTemporaryFile(delete=False, suffix=".mp3") as f:
            ruta = f.name
        await communicate.save(ruta)
        subprocess.run(["paplay", ruta], check=False)
    except Exception as error:
        print("❌ Error Edge TTS:", error)
    finally:
        if ruta:
            try:
                os.remove(ruta)
...
def hablar(texto):
    try:
        ...
        data = {"text": texto, "model_id": "eleven_monolingual_v1"}
        ...
        print("⚠️ ElevenLabs falló:", error)
        try:
...
def escuchar():
    ...
    try:
        ...
        print("❌ Error del servicio de reconocimiento:", error)
    except Exception as error:
        print("❌ Error mic:", error)
    return ""
...
def extraer_nombre(texto):
    ...
    if not texto:
        return None
    coincidencia = re.search(r"(?:me llamo|mi nombre es|soy)\s+([A-Za-zÁÉÍÓÚÜÑáéíóúüñ]{2,30})", texto, flags=re.IGNORECASE)
    if coincidencia:
...
def detectar_emocion(image_path):
    if not GEMINI_API_KEY:
        return "neutral"
    try:
        payload = {"contents": [{"parts": [{"text": "Analiza únicamente la expresión facial visible. Responde con una sola palabra en español de esta lista: feliz, triste, enojado, asustado, sorprendido, neutral."}, {"inlineData": {"mimeType": "image/jpeg", "data": image_to_base64(image_path)}}]}]}
        response = requests.post(GEMINI_API_URL, json=payload, timeout=30)
        response.raise_for_status()
        data = response.json()
        emocion = data["candidates"][0]["content"]["parts"][0]["text"].strip().lower()
        emociones_validas = {"feliz", "triste", "enojado", "asustado", "sorprendido", "neutral"}
...
Responde en español de manera corta, cálida, amigable, natural y apropiada para un niño.
...
"""
    try:
        response = groq_client.chat.completions.create(messages=[{"role": "user", "content": prompt}], model="llama-3.1-8b-instant", temperature=0.7, max_tokens=100)
        respuesta = response.choices[0].message.content.strip()
    except Exception as error:
        print("❌ Error Groq:", error)
...
def escuchar_mqtt():
    ...
    try:
        ...
        print("⚠️ No se pudo conectar al broker MQTT:", error)
        return None
...
def activar_robot():
    ...
    def run():
        ...
        try:
            ...
            hablar(f"Mucho gusto, {nombre}. Voy a acompañarte un momento." if nombre else "Mucho gusto. Voy a acompañarte un momento.")
            ...
            while True:
                ...
                if not texto:
                    ...
                    continue
                intentos = 0
                if any(frase in texto.lower() for frase in ("gracias", "adiós", "adios", "chau")):
                    hablar("Fue lindo hablar contigo. Nos vemos pronto.")
                    break
                hablar(generar_respuesta("conversación", texto))
        except Exception as error:
