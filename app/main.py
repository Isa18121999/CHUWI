...
class SupportProfileRequest(BaseModel):
    """Datos mÃ­nimos para adaptar el tono; no guardar diagnÃ³stico ni tratamiento."""
    alias: str
    rango_edad: str = "6-12"
    intereses: list[str] = []
...
async def register(
    ...
    user = Usuario(username=req.username, password_hash=hash_password(req.password), rol=req.rol)
    db.add(user)
    await db.commit()
...
async def robot_start(current_user: Usuario = Depends(get_current_user)):
    if robot.is_running:
        raise HTTPException(status_code=400, detail="Robot ya estÃ¡ activo")
    await robot.start()
...
async def robot_stop(current_user: Usuario = Depends(get_current_user)):
    if not robot.is_running:
        raise HTTPException(status_code=400, detail="Robot ya estÃ¡ detenido")
    await robot.stop()
    return {"message": "Robot detenido", "state": robot.state.value}


# ---- Perfiles de acompaÃ±amiento ----
...
async def list_support_profiles(
    db: AsyncSession = Depends(get_db),
    current_user: Usuario = Depends(get_current_user),
):
...
async def create_support_profile(
    req: SupportProfileRequest,
    db: AsyncSession = Depends(get_db),
    current_user: Usuario = Depends(get_current_user),
):
    if current_user.rol != "admin":
        raise HTTPException(status_code=403, detail="Solo administraciÃ³n crea perfiles")
...
async def activate_support_profile(
    profile_id: int,
    db: AsyncSession = Depends(get_db),
    current_user: Usuario = Depends(get_current_user),
):
    if robot.is_running:
        raise HTTPException(status_code=400, detail="DetÃ©n el robot antes de cambiar de perfil")
    ...
    if not profile.autorizado_por_adulto or not profile.revisado_por_equipo:
        raise HTTPException(
            status_code=400,
            detail="El perfil necesita autorizaciÃ³n del adulto y revisiÃ³n del equipo de salud",
        )
...
# ---- Registro clÃ­nico privado (doctor/admin) ----

def require_clinical_access(user: Usuario):
    if user.rol not in {"doctor", "admin"}:
        raise HTTPException(status_code=403, detail="Acceso reservado para personal clÃ­nico autorizado")
...
async def create_patient_registry(
    req: PatientRegistryRequest,
    db: AsyncSession = Depends(get_db),
    current_user: Usuario = Depends(get_current_user),
):
    require_clinical_access(current_user)
    if req.perfil_id and not await db.get(PerfilAcompanamiento, req.perfil_id):
        raise HTTPException(status_code=404, detail="Perfil de acompaÃ±amiento no encontrado")
    patient = PacienteRegistro(
        codigo_publico=f"temporal-{uuid4()}",
        nombre_completo=req.nombre_completo,
        identificador_clinico=req.identificador_clinico,
...
async def list_patient_registry(
    db: AsyncSession = Depends(get_db),
    current_user: Usuario = Depends(get_current_user),
):
    require_clinical_access(current_user)
...
async def open_patient_registry(
    patient_id: int,
    req: PatientAccessRequest,
    db: AsyncSession = Depends(get_db),
    current_user: Usuario = Depends(get_current_user),
):
    require_clinical_access(current_user)
    ...
    return {
        "id": patient.id,
        "codigo_publico": patient.codigo_publico,
        "nombre_completo": patient.nombre_completo,
        "identificador_clinico": patient.identificador_clinico,
        "perfil_id": patient.perfil_id,
    }
...
async def activate_patient(
    ...
    require_clinical_access(current_user)
    if robot.is_running:
        raise HTTPException(status_code=400, detail="DetÃ©n el robot antes de cambiar de paciente")
    patient = await db.get(PacienteRegistro, patient_id)
    if not patient:
        raise HTTPException(status_code=404, detail="Paciente no encontrado")
    ...
    if profile and (not profile.autorizado_por_adulto or not profile.revisado_por_equipo):
        raise HTTPException(status_code=400, detail="El perfil asignado aÃºn no estÃ¡ autorizado")
    robot.set_active_patient(
        patient.id,
        patient.codigo_publico,
...
async def get_sessions(
    ...
    if session_ids:
        links = await db.execute(
            select(SesionPaciente.sesion_id, PacienteRegistro.codigo_publico)
            .join(PacienteRegistro, PacienteRegistro.id == SesionPaciente.paciente_id)
            .where(SesionPaciente.sesion_id.in_(session_ids))
        )
        patient_codes = {session_id: code for session_id, code in links.all()}
    return [
        {
            "id": s.id,
            "paciente": patient_codes.get(s.id, "Paciente no asignado"),
...
async def mark_notification_read(
    ...
    notif.leida = True
    await db.commit()
    return {"message": "Marcada como leÃ­da"}
